...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!match_result_id || !response) throw new Error("Missing match_result_id or response");
    ...
    const employerEmail = employerProfile?.contact_email || employerUser?.user?.email;
