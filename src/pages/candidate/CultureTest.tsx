...
const CultureTest = () => {
  ...
  const fetchExistingAnswers = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select("culture_answers, culture_test_completed")
        .eq("user_id", user.id)
        .single();
      ...
      if (data?.culture_answers) {
        ...
        if (data.culture_test_completed) {
          calculateAndShowResults(data.culture_answers as Record<string, number>);
        }
      }
    } catch (error) {
  ...
  const saveProgress = async (completed: boolean = false) => {
    ...
    try {
      ...
      const updateData: any = { 
        ...
        culture_test_completed: completed,
      };
        ...
        <main className="container mx-auto px-4 py-8 max-w-2xl">
          <Card className="border-success/20">
            <CardHeader className="text-center">
              ...
              <CardDescription>Test kultury organizacji ukończony</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                  ...
                  return (
                    <div key={code} className="bg-muted/50 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-sm font-medium ${level === 'high' ? 'text-success' : level === 'medium' ? 'text-cta' : 'text-muted-foreground'}`}>
                          {score.toFixed(1)} / 5.0
                        </span>
                      </div>
              ...
              <div className="bg-accent/10 rounded-lg p-4 text-center">
                <p className="text-sm text-muted-foreground">
                  Te wyniki pomogą nam znaleźć organizację, której kultura pracy najlepiej odpowiada Twoim preferencjom.
                </p>
              </div>

              <Link to="/candidate/dashboard">
                <Button className="w-full bg-cta text-cta-foreground hover:bg-cta/90">
                  Kontynuuj do pytań dodatkowych
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-6">
          <h1 className="text-2xl font-bold mb-2">Test kultury organizacji pracy</h1>
